...
- [x] 1.1 `native/crates/lighthouse-core/src/policy.rs`: `Policy` struct
...
- [x] 1.2 TS twin `src/server/policy.ts`: same schema/semantics for
...
- [x] 4.1 `docs/managed-deployment.md`: example policy.json, per-OS
      distribution (GPO/Intune/Jamf/MDM snippets), the threat model
      paragraph, key reference table.
- [x] 4.2 `docs/data-flows.md`: note `telemetry:"off"` now exists and what
      it silences (update the "no runtime toggle" honesty note).
- [x] 4.3 E2E (the phase gate): with a temp policy file —
