...
| `chatHistory: "off"` | the history store's write path refuses (nothing persisted), existing saved chats stay readable (deleting user data on policy arrival would be destructive — the lock-not-wipe posture), toggle locked off. |
