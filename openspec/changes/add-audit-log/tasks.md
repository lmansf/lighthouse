...
- [x] 1.1 `native/crates/lighthouse-core/src/audit.rs`: `AuditRecord` struct
...
- [x] 1.2 `settings.rs`/`settings.ts`: add `audit_enabled: Option<bool>`
...
- [x] 2.1 Rust: capture the egress host-set + question sha at the start of
      ...
      share the assembly.
- [x] 2.2 TS twin: `audit.ts` (same record shape, NO hmac chain — PARITY)
      appended at the `app/api/chat` route's completion, same egress-delta
      approach over the TS registry.

## 3. Ops + viewer

- [x] 3.1 `rag_op`/routes `{op:"auditList"}` (recent N records) and
...
- [x] 3.2 Contracts: `RagService.audit()` (list + verify状态) — AuditSnapshot
      type; real via ops, mock returns disabled/empty.
- [x] 3.3 UI: an AuditLogDialog under SettingsMenu (recent records table:
...
- [x] 4.1 `docs/data-flows.md` note: the audit log is local-only, never an
      egress; `docs/managed-deployment.md` `auditLog` key already documented —
      cross-link the record shape + tamper model.
- [x] 4.2 E2E (phase gate): with audit on, a cloud-provider ask logs the
