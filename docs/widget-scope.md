...
| **W5 — Field fixes (0.4.2), verified against the live Linux shell** | ✅ done | Report from a real widget-mode user, each reproduced/fixed under Xvfb+openbox with `xprop`/screenshots: **(1) z-order** — the bar was created `always_on_top(true)`, dominating every window. Now it's a normal-stacking **resident** (new `WidgetResident` state = "blur keeps it around, but other windows may cover it"); the hotkey RAISES it (momentary topmost pulse + `set_focus` last, since GTK raises on present); pin is now the *only* thing that sets always-on-top. Verified: no `_NET_WM_STATE_ABOVE` at rest, present after a buried summon, ABOVE appears only when pinned. **(2) explorer blank/no-window** — `open_explorer`/`widget_show` were SYNC commands that build a webview, which deadlocks the IPC thread against the main loop; made them `async` + `run_on_main_thread`. Also stripped the stray GTK app menubar from both satellite windows. **(3) drag+pin** — the input filled the pill so there was no drag surface, and `data-tauri-drag-region` is a no-op in Tauri v2; added a dedicated grip calling `startDragging`, and pin now actually toggles always-on-top (was unconditional). **(4) dead ✕** — same sync-command deadlock class; the ✕→`widget_hide` path now lands (also releases a stale inline-answer hold). **(5) mode switch** — selecting widget mode now **hides the main window to tray** and summons the bar (window mode reverses it); the bar *replaces* the window as the surface in charge. Tray "Show Lighthouse" is always the way back. |
