...
| **W3 — Whisper mode (tap Ctrl+Super+Shift)** | 🚧 Windows landed | `whisper.rs`: WH_KEYBOARD_LL hook on a pump thread, TAP semantics (all three down + nothing else + first release → toggle; any other key marks the chord dirty so the keyed `+Space` hotkey and OS combos never double-fire). Opt-in `whisperMode` setting, live start/stop from Preferences (Windows-only switch). Cross-checked against the msvc target. Next rungs: macOS (flagsChanged + Accessibility consent) → X11; Wayland keeps the keyed chord. |
| **W4 — Polish & launch** | ⬜ not started | animations, multi-monitor placement, first-run hint, marketing copy. Carried-over review findings: widget-mode hides the main window, which makes the deferred launch-at-login prompt unreachable (fold startup consent into the chooser?); explorer window in no-bundle dev mode needs the loopback UI route + token story checked. |
