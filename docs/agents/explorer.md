...
- Read/write `src/stores/useRagStore.ts`: `sources`, `nodes`, `selectionMode`, `setSelectionMode`, `toggleIncluded`, `toggleSourceAvailable`, `upload`, `addReference`, `removeReference`, `load` (re-scan the vault), `includedFileIds()`.
