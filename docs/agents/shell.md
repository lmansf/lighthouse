...
## Scope (yours)
- `src/shell/` only: `AppShell.tsx`, `Sidebar.tsx`, `SidebarWater.tsx`, `ConversePlaceholder.tsx`, `theme.ts`, `dnd.ts`.
...
- When expanded, the sidebar also shows a **Converse** coming-soon placeholder (`ConversePlaceholder.tsx`) between the brand header and the file tree: a demand-gauging entry point for the future conversational mode (GitHub issue #66) that only opens a "coming soon" dialog. Its button carries `data-log` / `data-log-type` so the global click-capture records each press as a `click_events` row (`converse-coming-soon`), subject to usage-logging consent. Hidden while the sidebar is collapsed.
