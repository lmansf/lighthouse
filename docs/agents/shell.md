...
- The shell owns the **Converse** coming-soon placeholder (`ConversePlaceholder.tsx`): a demand-gauging entry point for the future conversational mode (GitHub issue #66) that only opens a "coming soon" dialog (no badge). The chat feature renders it in the chat header, immediately left of **New chat**. Its button carries `data-log` / `data-log-type` so the global click-capture records each press as a `click_events` row (`converse-coming-soon`), subject to usage-logging consent.
