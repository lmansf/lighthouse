...
5. **Attach files to a question** - drag a file from the explorer onto the panel, or drop OS files onto it, to scope the next question and its follow-ups to just those files. OS drops are handled the same way the explorer adds them: on the desktop they **link in place** (`@/shell/desktopBridge` `pathsForFiles` → `linkPaths`, no copy, and a file already covered by an existing link resolves to its node) and the linked *file* nodes are attached; anything without a resolvable path (plain browser) falls back to `upload`. Failures surface in a dismissible notice rather than vanishing silently. Attachments render as removable pills above the composer and clear on **New chat**. Use the shared `@/shell/dnd` helpers (`FILE_DRAG_MIME`, `parseDraggedFiles`) so internal drags stay distinguishable from OS file drops, and pass the attached ids as `attachmentFileIds` to `ask`.
6. **Converse entry point** - render the shell's `ConversePlaceholder` button in the chat header, immediately left of **New chat**. It only opens a "coming soon" dialog for the future conversational mode (no badge).
