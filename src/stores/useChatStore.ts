...
/**
 * The history listing (§22.2): every conversation, newest first. Until 0.15.0
 * this partitioned by investigation — the current context's chats first, other
 * contexts' after — so the History section could widen its view without letting
 * another context's chats shuffle ahead. With investigations gone there is one
 * context, so there is one order. Exported for unit tests
 * (test/historyNavUi.test.mjs).
 */
export function conversationsAllContexts(conversations: Conversation[]): Conversation[] {
  ...
  return {
    ...
    deleteConversation: (id) =>
      set((s) => {
        ...
        // or a fresh empty one when none is left.
