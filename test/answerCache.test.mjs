...
function freshState() {
  ...
  const state = path.join(home, ".rag-vault");
  mkdirSync(state, { recursive: true });
  process.env.LIGHTHOUSE_APP_STATE_DIR = state;
  return home;
}
...
/** Where the persistence gate mirrors the store. */
const cacheFile = (home) => path.join(home, ".rag-vault", "answer-cache.json");
...
/** The key layout, pinned against the RAW material rather than merely against
 *  itself — three optional components ("\nr:", "\nv:", "\ns:") were dropped in
 *  0.15.0 with the features that filled them, and this is what makes that safe:
 *  a key is byte-identical to the pre-deletion zero-preference, zero-view,
 *  zero-definition key, so cache entries written before the deletion still hit.
 *  PARITY: answer_cache.rs::key_from_parts builds the same string. */
test("the key's byte layout is unchanged by the 0.15.0 component removals", async () => {
  const { createHash } = await import("node:crypto");
  const sha256 = (x) => createHash("sha256").update(x, "utf8").digest("hex");
  const { keyFromParts } = cache;
  assert.equal(
    keyFromParts("q", "openai", null, [], "digest"),
    sha256("q:q\nc:digest\np:openai\nm:\na:"),
  );
  assert.equal(
    keyFromParts("q", "openai", null, ["b.csv", "a.md"], "digest"),
    sha256("q:q\nc:digest\np:openai\nm:\na:a.md\u0000b.csv"),
...
// --- Key composition over a real corpus (fixture shared with the Rust twin) -------

test("provider, model, attachment subset, and attachment CONTENT each re-key", () => {
  freshState();
  cache.resetStore();
  const CONV = "conv-key";
  const ids = [
    workspace.attach(CONV, "report.md", Buffer.from("quarterly revenue summary")).id,
    workspace.attach(CONV, "private.csv", Buffer.from("region,revenue\nNE,100\n")).id,
  ];

  const q = "What were Q3 sales?";
  const key = (question, provider, model, atts) =>
    cache.workspaceCacheKey(CONV, question, provider, model, atts);

  const base = key(q, "openai", "gpt-5-mini", []);
  assert.equal(key("  what   WERE q3 sales?! ", "openai", "gpt-5-mini", []), base);
  assert.notEqual(key("What were Q4 sales?", "openai", "gpt-5-mini", []), base);
  assert.notEqual(key(q, "anthropic", "gpt-5-mini", []), base);
  assert.notEqual(key(q, "openai", "gpt-5", []), base);

  const ab = key(q, "openai", "gpt-5-mini", ids);
  assert.equal(key(q, "openai", "gpt-5-mini", [ids[1], ids[0]]), ab, "subset order folds");
  assert.notEqual(ab, base);
  assert.notEqual(key(q, "openai", "gpt-5-mini", [ids[0]]), ab);

  // Attachment CONTENT re-keys: a further file changes the conversation's
  // candidate digest, so the whole-conversation key moves…
  const wholeBefore = key(q, "local", null, []);
  const subsetBefore = key(q, "local", null, [ids[0]]);
  workspace.attach(CONV, "addendum.md", Buffer.from("a late arrival"));
  assert.notEqual(key(q, "local", null, []), wholeBefore, "a new attachment re-keys");
  // …while a key SCOPED to a named subset is untouched by a file outside it.
  // That locality is the point of the content-hash digest: in the vault era ANY
  // change under the folder invalidated EVERY entry.
  assert.equal(key(q, "local", null, [ids[0]]), subsetBefore);

  // Two conversations holding byte-identical files key identically.
  workspace.attach("conv-twin", "report.md", Buffer.from("quarterly revenue summary"));
  workspace.attach("conv-twin", "private.csv", Buffer.from("region,revenue\nNE,100\n"));
  workspace.attach("conv-twin", "addendum.md", Buffer.from("a late arrival"));
  assert.equal(
    cache.workspaceCacheKey("conv-twin", q, "openai", "gpt-5-mini", []),
    key(q, "openai", "gpt-5-mini", []),
    "identical bytes ⇒ identical key, in any conversation",
  );

  // One changed byte anywhere in the corpus is a different key.
  workspace.attach("conv-diff", "report.md", Buffer.from("quarterly revenue summary!"));
  assert.notEqual(cache.workspaceCacheKey("conv-diff", q, "openai", "gpt-5-mini", []), base);

  // A null conversation is an EMPTY corpus, not a fallback to anything.
  assert.equal(
    cache.workspaceCacheKey(null, q, "openai", "gpt-5-mini", []),
    cache.workspaceCacheKey("conv-empty", q, "openai", "gpt-5-mini", []),
  );
});
...
test("history-off serves memory only and deletes the disk mirror", () => {
  ...
  assert.ok(existsSync(cacheFile(home)), "history-on mirrors to disk");

  // A disallowed ask still hits IN MEMORY — and removes the disk file.
  assert.equal(cache.lookup("k1", DISALLOWED)?.text, "one");
  assert.ok(!existsSync(cacheFile(home)), "history-off deletes the persisted cache");
  ...
  assert.ok(!existsSync(cacheFile(home)), "a bypassed disallowed ask still clears the mirror");
});
...
test("E2E: re-ask replays with ZERO provider calls; a changed corpus runs live", async () => {
  const home = freshState();
  cache.resetStore();
  ...
  try {
    ...
    assert.ok(!existsSync(cacheFile(home)), "memory-only by default — no disk mirror");
    ...
    workspace.attach(CONV, "margins.md", Buffer.from("Margins improved too."));
    const third = await ask();
    assert.equal(calls, 2, "a changed corpus invalidates — the provider ran again");
...
test("a handcrafted v:1 envelope is accepted, and inserts stamp v:1 back", () => {
  ...
  assert.equal(JSON.parse(readFileSync(cacheFile(home), "utf8")).v, 1, "writes stamp v:1");
});
...
test("the disk mirror replays across a REAL process restart (the latch starts unlatched)", () => {
  ...
  const r = spawnSync(process.execPath, ["--input-type=module", "-e", script], {
    env: { ...process.env, LIGHTHOUSE_APP_STATE_DIR: path.join(home, ".rag-vault") },
    encoding: "utf8",
  });
...
test("INSERT applies the posture too: allowed merges the prior store, disallowed deletes it", () => {
  ...
  const onDisk = JSON.parse(readFileSync(cacheFile(home), "utf8"));
  assert.ok(
  ...
  assert.ok(!existsSync(cacheFile(home)), "a disallowed insert deletes the persisted cache");
});
